    ...
    public function index($orderid, $paytype = '', $third_id = '') {
        ...
        if ($order = M('dish_order')->where(array('orderid' => $orderid))->find()) {
            ...
            if ($order['paid']) {
                ...
                $model->sendTempMsg('OPENTM202521011', array('href' => $siteurl.U('DishOut/myOrder', array('token' => $token, 'wecha_id' => $wecha_id, 'cid' => $order['cid'])), 'wecha_id' => $wecha_id, 'first' => '外卖订餐交易提醒', 'keyword1' => $orderid, 'keyword2' => date("Y年m月d日H时i分s秒"), 'remark' => '支付成功，感谢您的光临，欢迎下次再次光临！'));
