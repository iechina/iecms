...
// 		$set = M("Twitter_set")->where(array('token' => $this->token, 'cid' => $this->_cid))->find();
...
// 		$this->assign('alipayConfig', $alipayConfig);
	...
	public function commentSave()
	{
		...
		if (false !== $comment->create($data)) {
			unset($data['__hash__']);
			$comment->add($data);
			$this->success("评论成功", U('Store/myDetail',array('token' => $this->token,'wecha_id' => $this->wecha_id, 'cid' => $this->_cid, 'cid' => $this->_cid,'cartid' => $cartid, 'twid' => $this->_twid)));
		} else {
			$this->error($comment->error, U('Store/myDetail',array('token' => $this->token,'wecha_id' => $this->wecha_id, 'cid' => $this->_cid,'cartid' => $cartid, 'twid' => $this->_twid)));
		}
	}
...
// 				}
			...
			$this->redirect(U('Store/my',array('token' => $this->token,'wecha_id' => $this->wecha_id, 'cid' => $this->_cid, 'twid' => $this->_twid)));
	   }else{
	      exit('订单不存在');
	    }
	}
