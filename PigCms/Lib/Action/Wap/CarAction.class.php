...
public function sms(){
		...
		$this->reservebook_order=M('reservebook');
		$orders=$this->reservebook_order->where(array('token'=>$token,'type'=>'drive'))->order('date desc')->limit(0,1)->find();
		
	
		
		
			
			$str="\r\n姓名：".$orders['truename']."\r\n电话：".$orders['tel']."\r\n品牌/车系：".$orders['housetype']."\r\n车型：".$orders['choose']."\r\n预约日期：".$orders['dateline']."\r\n预约时间段：".$orders['timepart']."\r\n下单时间：".$orders['date']."\r\n";
    ...
    public function tool(){
        ...
        $utility = $t_uti->where(array('token'=>$token,'status'=>1))->order('id desc')->select();
    ...
    public function aboutus(){
        ...
        $company = M('Company');
        $token=$this->_get('token');
        $company_id = M('Carnews')->where(array('token'=>$token))->getField('company_id');
