...
// $info[9]['name'] = $setSchool['menu10'];
    ...
    public function readview(){
        ...
        if(isset($readtype) && $readtype == 'school'){
           ...
           $infolist= $t_res->where("token='$token' and type='$readtype' and id='$id'")->find();
    ...
    public function qresults(){
       $wecha_id  = trim(filter_var($this->wecha_id,FILTER_SANITIZE_STRING));
    ...
    public function mylist(){
        ...
        $where = array('token'=>$token,'wecha_id'=>$wecha_id,'type'=>$type,'rid'=>$rid);
