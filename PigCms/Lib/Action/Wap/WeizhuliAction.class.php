...
class WeizhuliAction extends WapAction
{
    ...
    public function index()
    {
		...
		$this->assign('a', $a);
        //dump($uip);exit;
        //微乾隆官方旗舰店:weiqianlong.taobao.com     ip判断写入
    ...
    public function reg()
    {
        ...
        $this->assign('zhuli', $zhuli);
		$info=M('weizhuli_user')->where(array('token'=> $token,'wecha_id'=> $wecha_id))->find();
        ...
        if (IS_POST) {
            ...
            if (empty($data['wecha_id'])) {
                ...
                $date['item_id'] = $ids;
                $date['trueip'] = $trueip;
                $enter = M('weizhuli_record')->add($date);}else{$this->error('您已经注册了，请不要重复注册！');}
            }
            if ($result) {
                $this->success('注册成功！', U('Weizhuli/index', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'ids' => $ids, 'action' => '1')));exit;
            } else {
                $this->error('注册失败！');
            }
        }else{$this->display();}
    ...
    public function sorts()
    {
        ...
        $where = array('token' => $token, 'id' => $id);
        $zhuli = $t_zhuli->where($where)->find();
        $t_zhuli = M('weizhuli_user')->where(array('token' => $token))->order('score desc')->limit(10)->select();
        //dump($t_zhuli);exit;
        $this->assign('zhuli', $zhuli);
