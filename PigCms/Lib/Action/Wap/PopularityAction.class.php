...
class PopularityAction extends WapAction{
	...
	public function index(){
		...
		if($this->popularity['is_attention'] == 1 && empty($this->wecha_id)) {
			$this->memberNotice('',1);
		}else if(($this->popularity['is_reg'] == 1 && empty($this->fans)) || ($this->popularity['is_attention'] == 2 && empty($this->wecha_id))) {
			$this->memberNotice();
		}
	...
	public function rank(){
		...
		$urank 	= array();
		$uinfo 	= M('Popularity_user')->where(array('token'=>$this->token,'pid'=>$id,'uid'=>$uid))->field('wecha_id,share_count')->find();
	...
	public function add_puser($wecha_id,$is_real=1){
		...
		$user = M('Popularity_user')->where(array('token'=>$this->token,'wecha_id'=>$wecha_id,'is_real'=>$is_real))->find();
		if(empty($user)){
