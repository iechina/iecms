...
/*              $owhere         = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'is_open'=>'1','cardid'=>$thisCard['id'],'start'=>array('lt',time()),'end'=>array('gt',time()));
    ...
    public function integral(){
        ...
        $thisCard=$this->_thisCard();
        $this->assign('thisCard',$thisCard);
		$is_use     = strval($this->_get('is_use','intval')?$this->_get('is_use','intval'):'0'); //自行修复
    ...
    public function coupon(){
        ...
        foreach($data as $key=>$value){
            ...
            $data[$key]['company']  = $value['company_id']?M('Company')->where('id='.$value['company_id'])->getField('name'):'所有门店';
        }
        ...
        if(IS_POST){
            ...
            if($pay_type == 1){
                ...
                $paypass    = $this->fans['paypass'];       
                    ...
                    $this->redirect(U('CardPay/pay',array('from'=>'Card','token'=>$this->token,'wecha_id'=>$this->wecha_id,'price'=>$arr['expense'],'single_orderid'=>$single_orderid,'orderName'=>'支付除优惠劵外多余款项','redirect'=>'Card/payReturn|itemid:'.$r_record['coupon_id'].',usecount:'.$arr['usecount'].',score:'.$arr['score'].',type:coupon,cardid:'.$cardid)));
                    exit;
