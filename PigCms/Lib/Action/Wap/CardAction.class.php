...
/*    			$owhere 		= array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'is_open'=>'1','cardid'=>$thisCard['id'],'start'=>array('lt',time()),'end'=>array('gt',time()));
	...
	public function get_card(){
		...
		if($get_card!=false){
			Header("Location: ".C('site_url').'/'.U('Wap/Card/vip',array('token'=>$this->_get('token'),'wecha_id'=>$this->wecha_id))); 
		}
		if($token!=false){
			//会员卡信息
			$data=M('member_card_set')->where(array('token'=>$token))->find();
	...
	public function info(){
		...
		if($token!=false){
			...
			$contact=M('member_card_contact')->where(array('token'=>$token))->order('sort desc')->find();
			//我的卡号
			$mycard=M('member_card_create')->where(array('token'=>$this->_get('token'),'wecha_id'=>$this->wecha_id))->find();
	...
	public function vip(){
	   ...
	   $agent = $_SERVER['HTTP_USER_AGENT']; 
		...
		if($token!=false){
			...
			$info=M('member_card_info')->where(array('token'=>$token))->find();
			//卡号
			$card=M('member_card_create')->where(array('token'=>$token,'wecha_id'=>$this->wecha_id))->find();
			//var_dump($card);exit;
	...
	public function consume(){
		...
		$config = M('Alipay_config')->where(array('token'=>$this->token))->find();
		$cardid = $this->_get('cardid','intval');
		$memberCard['cardid'] = $this->_get('cardid','intval');
	...
	public function payReturn(){
		...
		if($order){
			if($order['paid'] == 1){
                ...
                $info       = M('Member_card_set')->where(array('token'=>$this->token,'cardid'=>$cardid))->find();
                $cardinfo   = M('Member_card_create')->where(array('token'=>$this->token,'cardid'=>$cardid,'wecha_id'=>$this->wecha_id))->find();
