...
class SharetalentAction extends WapAction
{
    ...
    public function info()
    {
        ...
        if ($action == 'share') {
            ...
            $clickip['token'] = $token;
            $clickip['pid'] = $pid;
			$clickip['wecha_id'] = $wecha_id;
            $clickip['enterdate'] = time();
            $userip = M('sharetalent_userip')->where(array('token' => $token,'pid'=>$pid,'wecha_id'=>$wecha_id, 'ip' => $ip))->find();
            ...
            if (empty($s)) {
                ...
                if (empty($userip)) {
					...
					$click = M('sharetalent_record')->where(array('token' => $token, 'pid' => $pid, 'wecha_id' => $wecha_id))->setInc('click');
					 $s = M('sharetalent_record')->where(array('token' => $token, 'wecha_id' => $wecha_id, 'pid' => $pid))->find();
					 $second['end']=$s['click']-$s['rule'];
					  $sone = M('sharetalent_record')->where(array('token' => $token, 'wecha_id' => $wecha_id, 'pid' => $pid))->save($second);
                } elseif ($end >= 86400) {
