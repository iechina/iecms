...
//		$checkFunc->cfdwdgfds3skgfds3szsd3idsj();
		...
		case 'delorder':
			...
			if ($order_list != NULL) {
				foreach ($order_list as $vo) {
					...
					$where_cart['order_id'] = $vo['pigcms_id'];
	...
	public function dobuy()
	{
		...
		$update_order = $this->m_order->where(array('pigcms_id' => $id_order))->save($save_order);
		...
		if (0 < $id_order) {
			...
			if (0 < $update_cart) {
				$this->redirect('Alipay/pay', array('token' => $this->token, 'price' => $total, 'wecha_id' => $this->wecha_id, 'from' => 'Unitary', 'orderid' => $save_order['orderid'], 'single_orderid' => $save_order['orderid']));
			}
		}
	}
	...
	public function payReturn()
	{
		...
		if (isset($_GET['nohandle'])) {
			$this->redirect('Unitary/payend', array('token' => $this->token, 'orderid' => $order['pigcms_id']));
		}
		else {
