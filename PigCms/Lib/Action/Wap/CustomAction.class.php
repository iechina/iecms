...
public function sms(){
		...
		$this->selfform=M('custom_info');
		$orders=$this->selfform->where(array('token'=>$this->token,'id'=>$this->_get('id','intval')))->order('add_time desc')->limit(0,1)->find();
		
		
		
			
			$str="\r\n恭喜您有新的微预约订单\r\n提交时间：".date('Y-m-d H:i:s',$orders['add_time'])."\r\n";
