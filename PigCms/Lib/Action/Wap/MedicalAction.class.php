...
public function sms(){
		...
		$this->medical_order=M('medical_user');
		$orders=$this->medical_order->where(array('token'=>$token))->order('date desc')->limit(0,1)->find();
		
		if($orders['sex']=1){$sex='男';};
		if($orders['sex']=2){$sex='女';};
