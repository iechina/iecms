...
class ScenesAction extends UserAction{
	...
	public function _initialize() {
		...
		if(!strpos($token_open['queryname'],'Scene')){
	...
	public function add(){
		...
		if(IS_POST){ 
			...
			if($id =$this->Scene_model->add($_POST)){
				...
				//print_r($_POST);die;
					...
					$this->success('添加成功',U('Scenes/index',array('token'=>$this->token)));
	...
	public function index(){
		...
		$set=$this->Scene_model->where(array('token'=>$this->token))->select();
	...
	public function edit(){
		...
		$set=$this->Scene_model->where(array('token'=>$this->token,'id'=>$id))->find();
		$this->assign('set',$set);
		...
		if(IS_POST){
			...
			if($this->Scene_model->where(array('id'=>$id))->save($_POST)){
				$keyword_model=M('Keyword');

				
				//print_r($key);die;

				$keyword_model->where(array('token'=>$this->token,'pid'=>$id,'module'=>'Scenes'))->save(array('keyword'=>$_POST['keyword']));

				$this->success('修改成功！',U('Scenes/index',array('token'=>$this->token)));
	...
	public function del(){
		...
		$check=M('scene')->where(array('token'=>$_SESSION['token'],'id'=>$this->_get('id')))->find();
		
		if($check==false){$this->error('服务器繁忙');}
		if(empty($_POST['edit'])){
			if(M('scene')->where(array('id'=>$check['id'],'token'=>$_SESSION['token']))->delete()){
				M('scene_addtp')->where(array('pid'=>$check['id'],'token'=>$_SESSION['token']))->delete();
				M('keyword')->where(array('pid'=>$check['id'],'token'=>$_SESSION['token'],'module'=>'Scene'))->delete();
...
public function addtp(){
		...
		$checkdata=M('scene')->where(array('token'=>$_SESSION['token'],'id'=>$this->_get('id')))->find();
		...
		//upyun多文件上传
			...
			$info=M('scene_addtp')->where(array('token'=>$_SESSION['token'],'pid'=>$this->_get('id')))->order('sort desc')->select();
	...
	public function list_edit(){
		if($this->_get('token')!=session('token')){$this->error('非法操作');}
		$check=M('scene_addtp')->field('id,pid')->where(array('token'=>$_SESSION['token'],'id'=>$this->_post('id')))->find();
		if($check==false){$this->error('照片不存在');}
		
		if(IS_POST){
			
			$this->all_save('scene_addtp','/addtp?id='.$check['pid']);		
		}else{
			$this->error('非法操作');
		}
	}
	...
	public function edittp(){
		$info=M('scene_addtp')->where(array('token'=>$_SESSION['token'],'id'=>$this->_get('id')))->order('sort desc')->select();
...
public function list_del(){
		if($this->_get('token')!=session('token')){$this->error('非法操作');}
		$check=M('scene_addtp')->field('id,pid')->where(array('token'=>$_SESSION['token'],'id'=>$this->_get('id')))->find();
		
		if($check==false){$this->error('服务器繁忙');}
		if(empty($_POST['edit'])){
			if(M('scene_addtp')->where(array('id'=>$check['id']))->delete()){
