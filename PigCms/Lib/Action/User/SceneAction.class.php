...
*/
	...
	public function getConnectNum(){
		...
		//$user 	= $this->_getMember('shake_id',$this->_post('id','intval'));
		//echo count($user);
		echo $count;
	}
	...
	public function shakeRun(){
		...
		$result 	= array();
		if($is_end || (($shake_info['endtime']+$shake_info['usetime']+$shake_info['starttime'])<time())){  //游戏是否结束
...
*/
	...
	public function get_lottery(){
		...
		$info 	= M('Wall_member')->where($iwhere)->order('time desc')->limit(50)->select();
