		...
		if(IS_POST){
			...
			if($prize_db->create()){
				...
				if(empty($packet_info)){
					...
					$this->handleKeyword($id,'Packet',$this->_post('keyword','trim'));
					...
					$this->handleKeyword($this->_post('id','intval'),'Packet',$this->_post('keyword','trim'));
...
/*********/
	public function prize_log(){
		...
		foreach ($list as $key=>$value){
			//$reward = M('Red_packet_reward')->where(array('token'=>$this->token,'log_id'=>$value['id']))->find();
			//$list[$key]['mobile'] 	= $reward['mobile'];
