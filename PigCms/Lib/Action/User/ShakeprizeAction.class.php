<?php
class ShakeprizeAction extends UserAction{    
    public function _initialize() {
		parent::_initialize();
		$function=M('Function')->where(array('funname'=>'Shakeprize'))->find();
		$this->canUseFunction('Shakeprize');
	}    
	public function index(){
		if(session('gid')==1){
			$this->error('vip0无法使用抽奖活动,请充值后再使用',U('Alipay/index'));
		}
		...
		foreach ($list as $key => $val){
			$list[$key]['joinnum'] = M('Lottery')->where(array('token'=>session('token'),'type'=>6,'lid'=>$val['id']))->count();
		}
		...
		if(IS_POST){		
			...
			if($data->create()!=false){				
				if($id=$data->add()){
					...
					$this->success('活动创建成功',U('Shakeprize/index'));
				}else{
	...
	public function setinc(){
		if(session('gid')==1){
			$this->error('vip0无法开启活动,请充值后再使用',U('Alipay/index'));
		}
		$id=$this->_get('id');
		...
		if($user['activitynum']>=$group['activitynum']){
			$this->error('您的免费活动创建数已经全部使用完,请充值后再使用',U('Alipay/index'));
		}
