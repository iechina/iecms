...
//删除记录
	...
	public function center(){
		...
		foreach($allcard as $key=>$value){
			...
			$allcard[$key]['username']	= $user_info['truename'];
	...
	public function coupons_record(){
        ...
        $list 	= M('Member_card_coupon_record')->alias('record')
            ...
            ->where($where)
            ->field('record.id,record.add_time,record.staff_id,record.company_id,record.cancel_code,record.use_time,record.is_use,record.coupon_type,record.where,record.whereid,coupon.title,coupon.is_weixin,user.wechaname,user.tel')
