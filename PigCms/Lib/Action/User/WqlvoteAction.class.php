<?php
class WqlvoteAction extends UserAction{


    public function index(){
        //$this->canUseFunction('Wqlvote');
       ...
       // $type = isset($this->_get('type')) ? $this->_get('type') : 'text';
        ...
        $list=M('Wqlvote')->where($where)->order('id DESC')->select();
        $count = M('Wqlvote')->where($where)->count();
    ...
    public function totals(){
        ...
        $Wqlvote_item = M('WqlWqlvote_item')->where('vid='. $vote['id'])->select();
        $vcount = $t_record->where(array('vid'=>$id))->count();
        $this->assign('count',$vcount);
        $item_count = M('Wqlvote_item')->where('vid='.$id)->select();
        ...
        $this->assign('xml',$xml);
        $this->assign('Wqlvote_item', $Wqlvote_item);
    ...
    public function del_record(){
        ...
        $record_info = M('Wqlvote_record')->where(array('token'=>$this->token,'id'=>$id))->find();
        if(M('Wqlvote_record')->where(array('id'=>$id))->delete()){
            M('Wqlvote_item')->where(array('id'=>array('in',"{$record_info['item_id']}")))->setDec('vcount',1);
            M('vote')->where(array('token'=>$this->token,'id'=>$record_info['vid']))->setDec('count',1);
 
            $this->success('删除成功',U('Wqlvote/index',array('token'=>session('token'))));
        }
    ...
    public function _getItemName($item_id){
        ...
        foreach ($id as $key => $value) {
            if($value){
                $name .= M('Wqlvote_item')->where('id='. $value)->getField('item').',';
            }
        }
            ...
            if($data->create()!=false){
                if($id=$data->add()){
                    ...
                    if($this->_get('type') != 'scene'){
                        $this->handleKeyword($id,'Wqlvote',$this->_post('keyword','trim'));
                    }
                    //$ukeywordser=M('Users')->where(array('id'=>session('uid')))->setInc('activitynum');
                    $this->success('添加成功',U('Wqlvote/index',array('token'=>session('token'))));
                }else{
    ...
    public function del(){
        ...
        $result = $vote->where($find)->find();
         if($result){
            ...
            M('Wqlvote_item')->where('vid='.$result['id'])->delete();
            M('Wqlvote_record')->where('vid='.$result['id'])->delete();
            $where = array('pid'=>$result['id'],'module'=>'Wqlvote','token'=>session('token'));
    ...
    public function setinc(){
        $id=$this->_get('id');
        $where=array('id'=>$id,'token'=>session('token'));
        $check=M('Wqlvote')->where($where)->find();
        ...
        if ($check['status']==0){
            $data=M('Wqlvote')->where($where)->save(array('status'=>1));
            $tip='恭喜你,活动已经开始';
        }else {
            $data=M('Wqlvote')->where($where)->save(array('status'=>0));
            $tip='设置成功,活动已经结束';
        }
    ...
    public function setdes(){
        ...
        $check=M('Wqlvote')->where($where)->find();
        if($check==NULL)$this->error('非法操作');
        $data=M('Wqlvote')->where($where)->setDec('status');
        if($data!=NULL){
            $this->success('活动已经结束');
        }else{
            $this->error('服务器繁忙,请稍候再试');
        ...
        if(IS_POST){
            ...
            if($data->create()){
                ...
                if($data->where($where)->save($_POST)){
                    if($this->_get('type') != 'scene'){
                        $this->handleKeyword($this->_post('id','intval'),'Wqlvote',$this->_post('keyword','trim'));
                    }

                    $this->success('修改成功!',U('Wqlvote/index',array('token'=>session('token'))));exit;
                }else{
            ...
            $items = M('Wqlvote_item')->where('vid='.$id)->order('rank DESC')->select();
    ...
    public function del_tab(){
         ...
         M('Wqlvote_item')->where(array('id'=>$da['tid']))->delete();
