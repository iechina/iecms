		...
		if ($thisOrder = $dishOrder->where(array('id' => $id, 'cid' => $cid, 'token' => $this->token, 'comefrom' => 'dishout'))->find()) {
			if (IS_POST) {
				...
				if ($isuse) {
					...
					$model->sendTempMsg('TM00820', array('href' => U('DishOut/myOrder', array('token' => $this->token, 'wecha_id' => $thisOrder['wecha_id'], 'cid' => $thisOrder['cid'])), 'wecha_id' => $thisOrder['wecha_id'], 'first' => '外卖送菜提醒', 'keynote1' => '您订的外卖已送出', 'keynote2' => date('Y年m月d日H时i分s秒'), 'remark' => '您订的外卖已送出，请注意接收！'));
				}
