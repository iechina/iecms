...
class IndexAction extends UserAction{
	...
	//易企秀帐号同步
	public function eqx(){
		 $eqx_User = M('users','cj_','mysql://'.C('eqxuser').':'.C('eqxpassword').'@localhost/'.C('eqxname').''); 
	...
	public function upsave(){
		...
		M('Diymen_set')->where(array('token'=>$this->token))->save(array('appid'=>trim($this->_post('appid')),'appsecret'=>trim($this->_post('appsecret')),'telphone'=>trim($this->_post('telphone')),'password'=>trim($this->_post('password'))));
		//
		$db=D('Wxuser');
