...
class SharetalentAction extends UserAction{
	...
	public function add(){
		 ...
		 if(IS_POST){
            ...
            $data['info'] = str_replace('\'','&apos;',$_POST['info']);
			...
			if($insert){
                    ...
                    $this->success('添加成功',U('Sharetalent/index',array('token'=>session('token'))));exit;}
                    else{
	...
	//	dump($info);exit;
		$this->assign('info',$info);
		  if(IS_POST){
			...
			$data['prize'] = $this->_post('prize');
			$data['number'] = $this->_post('number');
			$data['rule'] = $this->_post('rule','stripslashes,htmlspecialchars_decode');
			$data['hdrules'] = $this->_post('hdrules','stripslashes,htmlspecialchars_decode');
			$data['picurl1'] = $this->_post('picurl1');
			$data['url'] = $this->_post('url');
			...
			if($insert){
                    ...
                    $this->success('更新成功',U('Sharetalent/index',array('token'=>session('token'))));exit;}
                    else{
                   $this->error('服务器繁忙,请稍候再试');exit;
                }}
 ...
 public function reply(){
		...
		if(IS_POST){
			...
			if($info){
				...
				if($result){
					$this->success('回复信息更新成功!');exit;
				}else{
					$this->error('服务器繁忙 更新失败!');exit;
				}
			}else{
				$data['token'] = session('token');
				$insert = M('sharetalent_reply')->add($data);
				if($insert > 0){
					$this->success('回复信息添加成功!');exit;
				}else{
					$this->error('回复信息添加失败!');exit;
				}
			}
		}else{
			$this->display();
   ...
   /**删除活动**/
    public function del(){
		...
		if($info){
                    ...
                    $this->success('删除成功',U('Sharetalent/index',array('token'=>session('token'))));exit;}
                    else{
		...
		public function sm(){
		 ...
		 if(IS_POST){
            ...
            $data['info'] = $this->_post('info','stripslashes,htmlspecialchars_decode');
			$data['infos'] = $this->_post('infos','stripslashes,htmlspecialchars_decode');
			if(empty($info)){
			      $insert = M('sharetalent_sm')->add($data);
			      if($insert){
               
                                 $this->success('添加成功',U('Sharetalent/sm',array('token'=>session('token'))));exit;}
                           else{
				 ...
				 $insert = M('sharetalent_sm')->where(array('token'=>session('token')))->save($data);	   
                    ...
                    $this->success('更新成功',U('Sharetalent/sm',array('token'=>session('token'))));exit;}
                    else{
                   $this->error('服务器繁忙,请稍候再试'); }  
	...
	public function sendprize(){
		...
		if($info){
		   ...
		   $newdates = M('sharetalent')->where(array('token'=>$token,'id'=>$pid))->save($new);
                    ...
                    $this->success('兑奖成功',U('Sharetalent/prize',array('token'=>$token,'wecha_id'=>$wecha_id,'id'=>$pid)));exit;}
                    else{
                   $this->error('兑奖失败,请稍候再试',U('Sharetalent/prize',array('token'=>$token,'wecha_id'=>$wecha_id,'id'=>$pid)));exit;
                }
	...
	public function userdel(){
		...
		$condition['token'] = $token;
		$condition['id'] = $id;
        $info = M('sharetalent_user')->where($condition)->delete();
		
       if($info){
         
                    $this->success('删除成功',U('Sharetalent/user',array('token'=>$token,'wecha_id'=>$wecha_id,'id'=>$pid)));exit;}
                    else{
                   $this->error('删除失败,请稍候再试',U('Sharetalent/user',array('token'=>$token,'wecha_id'=>$wecha_id,'id'=>$pid)));exit;
                }
