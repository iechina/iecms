...
<!--背景音乐end-->
	...
	function startRequest(){
		$.post("<?php echo U('Index/ajaxlist');?>",
	   function(data){
			...
			$objct=eval(data);
			  ...
			  $($objct).each(function(d){
				...
				if('undefined' != typeof($('#'+$opnid))){
					...
					$str='<div class="chatListColumn" id="'+$opnid+'" onclick="selected(\''+$opnid+'\',1)"> <div class="clicked" style="display:none;"></div><span class="unreadDot" style="display:none">0</span> <span class="unreadDotS" style="display:none"></span><div class="avatar_wrap"><img class="avatar" src="index.php?g=Chat&m=&a=showExternalPic&token=<?php echo ($token); ?>&wecha_id='+$opnid+'&url='+$headimgurl+'" /></div><span id="showNums'+$opnid+'" class="unreadDot" style="display: block;">1</span><div class="extend"><p class="time">'+$subscribe_time+'</p>   <div class="edited"><i class="editedIcon"></i></div> <div class="mute" style="display:none;"></div></div><div class="info"> <div class="nickName"><div class="left name" style="">'+$nickname+'</div><div class="clr"></div></div> <div class="descWrapper"> <p class="desc">'+$city+'</p> </div></div> <div class="clr"></div> </div>';
		clickMisic();
	...
	<div id="chat" class="chatPanel normalPanel" ctrl="1">
		...
		<div class="content">
				...
				<div class="top" >
					<div id="profile" class="title"> 
						<div class="myProfile"> 
							...
							<div class="info"> 
								<span class="nickName" id='wqvb'><?php echo (session('name')); ?></span> 
					...
					<div style="height: 100%; width: 100%;  position: relative; display: block;">
						<div class="listContent" id="chat_conversationListContent" style="width: 100%; top: 0px; position: absolute;" ctrl="1">
							<div id="conversationContainer">
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="chatListColumn" id="<?php echo ($list["openid"]); ?>" onClick="selected('<?php echo ($list["openid"]); ?>',<?php echo ($i); ?>)"> 
									...
									<div class="avatar_wrap">
										<img class="avatar" src="<?php if($list['headimgurl'] != ''): ?>index.php?g=Chat&m=&a=showExternalPic&token=<?php echo ($token); ?>&wecha_id=<?php echo ($list["openid"]); ?>&url=<?php echo ($list["headimgurl"]); else: ?>/tpl/Chat/default/style/css/img/avatar_default17ced3.png<?php endif; ?>" />
									</div>
									<div class="extend">
										<p class="time"><?php echo ($list["endtime"]); ?></p>   
										<div class="edited">
									...
									<div class="info"> 
										<div class="nickName">
											<div class="left name" style=""><?php echo (($list["nickname"])?($list["nickname"]):'新客户-'); ?></div>
											<div class="clr"></div>
										</div> 
										<div class="descWrapper"> 
											<p class="desc"><?php echo ($list["city"]); ?>－<?php echo ($list["subscribe_time"]); ?> </p> 
										</div>
									</div> 
									<div class="clr"></div> 
			...
			<div class="chat lightBorder" style="visibility: visible;">
				...
				<iframe src="<?php echo U('Index/main');?>#mian2" id="iframepage" name="iframepage" frameBorder=0 scrolling=no width="100%" height="520" onLoad="iFrameHeight()" ></iframe> 
