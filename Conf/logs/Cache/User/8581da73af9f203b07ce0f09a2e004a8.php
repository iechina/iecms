...
<tr>
   ...
   <tbody>
	  ...
	  <?php if(is_array($class['class'])): $i = 0; $__LIST__ = $class['class'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$class1): $mod = ($i % 2 );++$i;?><tr class="td29">
			...
			<td colspan="1">
			<a class="ajax btnGreen cboxElement" href='javascript:tipsWindown("修改菜单","iframe:<?php echo U('Diymen/class_edit',array('id'=>$class1['id']));?>","760","480","true","","true","leotheme");' title="修改主菜单">修改</a>
	...
	<tr class="hover">
		...
		<?php if($class != false): ?><a class="btnGreen " onclick="drop_confirm('自定义菜单最多勾选3个，每个菜单的子菜单最多5个，请确认!', '<?php echo U('Diymen/class_send');?>');" title="">生成自定义菜单</a>

			<?php if($fuwuAppid != ''){ ?>
				<a class="btnGreen " onclick="drop_confirm('自定义菜单最多勾选4个，每个菜单的子菜单最多5个，请确认!', '<?php echo U('Fuwu/Button/dataReturn');?>');" title="">生成服务窗自定义菜单</a>
			<?php } endif; ?>
